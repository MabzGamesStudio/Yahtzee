		...
		for (int i = iteration; i < iterationStop + 1; i++)
		{
			...
			for (int j = 0; j < 64; j++)
			{
				...
				if (GameState.GameStatePossible(gameState1))
				{
					...
					// This adds the GameState IDs and values to lists
			...
			// The IDs and values lists are added to the yahtzee file in a 128ish element chunk to save on data add time
